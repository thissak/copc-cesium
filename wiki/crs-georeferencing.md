...
COPC 헤더의 WKT(좌표계 정의)를 읽어 projected X/Y를 WGS84 경위도로 reproject하고, 그 경위도를 ECEF로 올려 Cesium에 배치한다 — [[decode-in-worker]] 의 디코드 경로가 점마다 이 변환을 적용한다. WKT가 없거나 못 읽으면 좌표를 그대로 통과시키지(=지구 밖) 않고 **명확히 throw**하며, 사용자는 `crs`(강제)·`defaultCrs`(없을 때만) 옵션으로 좌표계를 직접 지정한다.
...
연결: [[decode-in-worker]](점마다 reproject가 도는 곳) · [[range-coalescing]](같은 source 계층의 IO 레버)
