...
async function measure(browser: Browser, url: string, reassert: boolean, setCacheMb?: number) {
...
async function main() {
  ...
  try {
    ...
    const eptium = await measure(browser, `https://viewer.copc.io/?copc=${DATASETS[ds]}`, true);
    ...
    const md = [
      `# 매칭 예산 head-to-head — ${ds} (깊은 0.15r, 같은 config, 정상상태 plateau, 실 GPU)`,
      ``,
      `GL: ${eptium.gl} · msse=${MSSE} · ⚠️ **매칭 작동점 한정 — headline 금지**`,
      ``,
    ...
    writeFileSync(`docs/bench/budget/matched-${ds}.md`, md);
    writeFileSync(`docs/bench/budget/matched-${ds}.json`, JSON.stringify({ ds, msse: MSSE, target, eptium, oursRuns, matched, mismatch, ratio }, null, 2));
    console.log(`[matched] wrote docs/bench/budget/matched-${ds}.{md,json}  ratio=${ratio.toFixed(3)} mismatch=${(mismatch * 100).toFixed(1)}%`);
