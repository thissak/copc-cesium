...
### 핵심 발견 — "빠른가/느린가"는 잘못 던진 질문
**두 엔진은 같은 화질 목표(SSE)에 17~37× 다른 점 수를 선택한다.**
- **ours = 고디테일/고비용** (기본 공격적 refine, 같은 설정에 17~37× 많은 점 = 훨씬 많은 GPU 작업)
- **Eptium = 저디테일/저비용** (보수적 refine, 점 예산 작음)

→ **§v4의 "동급/2×"는 712k로 매칭한 작동점에서만 성립**했고, 그건 ours를 기본보다 훨씬 높은 msse로 *억지로 throttle*한 상태였다. 실제 기본 설정에선 워크로드가 17~37× 다르므로 **단일 공정 verdict는 불가능**하다. (겹친 2버킷서 ours per-point ~0.5× gpuMs였으나 62.9% 노이즈로 확정 불가.)
...
- 미해결(후속): refine 공격성(geometricError 매핑)이 Eptium 대비 적정한지 — 디테일/비용 트레이드오프 재평가. fair-compare 노이즈(가파른 곡선·버킷)와 overlap(점 예산 격차)은 큰 재설계 필요라 보류.
