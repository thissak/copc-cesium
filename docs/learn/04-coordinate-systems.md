...
포인트클라우드를 화면에 "그리는" 것과 지구 위 "정확한 위치에 앉히는" 것은 다른 문제입니다. 후자가 **georeferencing**이고, 포인트클라우드 뷰어 버그의 가장 흔한 출처입니다.
...
**투영**은 둥근 지구를 평면 미터 격자로 펴는 것입니다. 국소 영역에서 거리·면적이 정확해 측량·포인트클라우드에 적합하지만, 영역마다 다른 투영(예: UTM zone)을 씁니다.

!!! note "EPSG 코드"
    각 CRS에는 `EPSG:번호` 식별자가 붙습니다. 좌표를 다룰 때 "이 데이터의 EPSG가 뭔가"가 항상 첫 질문입니다.

## 포인트클라우드의 좌표계는 보통 투영좌표다

포인트클라우드는 미터 단위 정밀 작업이 많아 **투영좌표(로컬 미터)**로 저장되는 게 일반적입니다. 그 CRS 정보는 LAS 파일 안에 **WKT**(또는 GeoTIFF VLR)로 들어 있습니다.
...
```mermaid
flowchart LR
    A["포인트클라우드 좌표<br/>(투영 CRS, 미터)<br/>X·scale+offset"] --> B["경위도<br/>(lon, lat, h)"]
...
포인트클라우드가 "안 보인다"의 대부분은 렌더가 아니라 **위치가 틀려서**입니다. 증상과 원인:
...
!!! warning "높이 기준 (height datum)"
    **타원체고(ellipsoidal height)** = 수학적 타원체 기준. **정사고도(orthometric height)** = 해수면(지오이드) 기준. Cesium은 기본적으로 타원체고를 기대합니다. 포인트클라우드가 정사고도면 **지오이드 보정**(예: EGM 모델)이 필요하며, 안 하면 수십 미터 위/아래로 어긋납니다.
