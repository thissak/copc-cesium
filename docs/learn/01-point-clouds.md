# 01. 점군(Point Cloud) 기초

## 점군이란

**3D 공간에 흩뿌려진 점들의 집합**입니다. 각 점은 위치 `(X, Y, Z)`와 여러 **속성(attribute)**을 가집니다. 면(face)도, 변(edge)도, 연결 정보(토폴로지)도 없습니다 — 오직 독립적인 점들의 나열입니다.

이 "토폴로지 없음"이 점군의 모든 성격을 결정합니다. 메시는 적은 수의 정점으로 큰 표면을 표현하지만, 점군은 표면을 **점의 밀도**로만 암시합니다. 그래서 같은 장면을 표현하는 데 압도적으로 많은 데이터가 필요합니다.
...
LiDAR는 "레이저를 쏘고 돌아오는 시간으로 거리를 잰다"를 초당 수십만~수백만 번 반복합니다. 그 결과가 점군입니다.
...
그래서 점군 기술의 거의 모든 노력은 두 가지로 수렴합니다:
...
!!! tip "성능 직관"
    점군 렌더는 **vertex 수**(점 개수)와 **fill-rate**(point size·overdraw) 두 압력을 받습니다. 둘 중 무엇에 묶였는지 가르는 게 [4축 병목 진단](../PROFILING.md)의 ④ GPU 축입니다.

---

→ 다음: [02. COPC 포맷](02-copc.md) — 이 무거운 점군을 "변환 없이 웹 스트리밍"으로 만든 파일 포맷.
