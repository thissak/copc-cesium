...
**LAS**는 ASPRS(미국 사진측량·원격탐사학회)가 정한 LiDAR 점군 표준 바이너리 포맷입니다. 구조:
...
**EPT(Entwine Point Tiles)**는 점군을 **옥트리**로 재구성해 LOD·스트리밍을 가능케 했습니다. 하지만 결과물이 **디렉토리 + 수천~수만 개의 작은 파일 + 인덱스**입니다. 관리·배포·전송(파일마다 HTTP 요청)이 번거롭습니다.
...
즉 EPT의 "옥트리 LOD"를 LAZ "단일 파일" 안에 집어넣은 것입니다. 별도 사이드카 없이 파일 하나. 래스터 세계의 **COG(Cloud Optimized GeoTIFF)** 와 같은 발상의 점군 버전입니다.
