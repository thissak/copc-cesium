...
**같은 문제**(웹에서 COPC 점군을 LOD 스트리밍으로 렌더)를 **이미 오픈소스로 푼 프로젝트들**. 단 렌더 엔진이 우리와 다르다.
...
- 점군 LOD 렌더 — point budget, 거리 기반 LOD, 속성(intensity/classification/색) 필터.
...
1. **점군 병목은 렌더러와 거의 무관.** [4축](PROFILING.md) 중 ①fetch ②decode ③CPU는 Cesium이든 Three.js든 동일하게 겪는다 (둘 다 WebGL로 수백만 점을 민다).
